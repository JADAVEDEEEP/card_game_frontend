...
export function Events() {
  ...
  useEffect(() => {
    ...
    return () => {
      document.getElementById("ep-global-css")?.remove();
    };
  }, []);
  ...
  const loadData = async () => {
    try {
      ...
      const response = primary.status === 404 ? await fetch(withApiBase("/watcher.js/recent?limit=all")) : primary;
  ...
  return (
    <section
      ...
      style={{ background: "#0a0a0f", borderRadius: 16, padding: "28px 24px 36px" }}
    >
      {/* Header */}
      <div className="ep-header">
        ...
        >
          Latest
          <br />
          <span style={{ color: "#6366f1" }}>Information</span>
        </h2>
